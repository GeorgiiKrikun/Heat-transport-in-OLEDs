...
While the electrical performance is governed by thermally activated charge transport and radiative recombination, enhanced currents cause more Joule heating and non-radiative recombination, making it nearly impossible to decouple.
%Due to the peculiar coupling between thermal and charge transport in organics, these properties can not be decoupled from each other.
%

Here we present a three-dimensional drift-diffusion model that self-consistently couples charge and heat transport. 
Using simulations based on this model, we establish how parameters responsible for heat transport, i.e. (i), the thermal conductivity of the organic layers and (ii) the heat transfer coefficient between the device surface and the environment govern the device operating temperature.
...
Intentionally elevated operating temperatures, that may improve the OLEDs electric performance, are not necessarily beneficial, as any increase in operating temperature comes for the price of decreased stability. 
Moreover, for any realistic device geometry thermal conductivities of organic layers will not be a bootleneck in heat transport, due to their low thickness. 
This allows us to attribute electric and thermal effects to the different device layers, which should be useful both for device modeling purposes and for device manufacturing (because different desired properties can be tuned separately).
Finally, we propose several analytic equations, which should provide better grasp on self heating and its consequences.
...
Therein, $\mathbf{j_n}$ and $\mathbf{j_p}$ are the electron and hole flux (rather than the electric current) respectively, $G$ and $R$ the generation and recombination rates. We are using Langevin recombination model.\cite{Langevin1903} 
...
The system of equations (\ref{eq:cont} and \ref{eq:poisson}) with the above-stated boundary conditions is solved in the regions of the organic semiconducting layers. 
Periodic boundary conditions are imposed along the spatial directions perpendicular to the stacking direction.
We discretized the equations (\ref{eq:cont} and \ref{eq:poisson}) with the Finite Differences Method on a non-uniform rectangular 3D mesh (for drift-diffusion equation we use the Scharfetter-Gummel discretization scheme \cite{SG}) and solved them iteratively with an explicit time discretization scheme (Euler forward) until steady state was achieved.
%%%%%%%%
...
Characteristic heat density distributions for three different values of $\kappa$ = $10^{-2}$, $10^{-3}$, $10^{-4}$~\thermalconductivity~and $h$~=~$10^{4.25}$~\hcoefficient  are shown in Fig. \ref{fig:setup}-b.
...
Elevated maximum temperatures occur if either $h$ or $\kappa$ are small. Small values of $h$ and $\kappa$ correspond to a situation, in which we reduce the ability to dissipate the heat from the OLED and, hence, store more heat inside.
...
Therefore, term in the inner bracket of Eq.~(\ref{eq:GreenFunctionHeatEquation}) should be equal to the $T(z)$ to fulfill Eq.~(\ref{eq:1Dheatequation}) in the presence of the heat distributed as $H(z)$. 
...
Where $z_{j'}=\sum_{i=1}^{j'} L_i$, $z_{0}=0$.
The Greens function $G(z,z_0)$ can then be conveniently written using functions $W^L(z)$ and $W^R(z)$ %in equations \ref{eq:greenfunction}.
...
To obtain temperature distribution in certain layer $j$ at point $z$ for given heat distribution $H(z_0)$ according to Eq.~(\ref{eq:GreenFunctionSolution}), one should integrate $H(z_0)$ weighted with the corresponding Green function $G_{j,j'}(z,z_0)$ with respect to $z_0$ and sum contributions from all layers $j'$. Exactly in this way we obtained Eqs. (\ref{eq:analt}) and (\ref{eq:onlyh}).
...
Here we intend to identify for which $h$ and $\kappa$ it is safe to assume that the electro-thermal coupling is not crucial.
For situations in which this assumption is invalid, we want to gather the symptoms of the electro-thermal feedback.
We check for the presence of feedback by comparing the temperature distribution $T_{sim}(z)$ from the full simulation and $T_{an}(z)$ from solving the steady state heat transport equation, Eq.~(\ref{eq:1Dheatequation}).
...
Our strategy was to extract heating profile from the simulation, where electro-thermal feedback is almost absent and then feed it into analytic equations. %I added this sentense to smooth transition to the next one
...
Not only the two temperatures $T_{max,sim}$ and $T_{max,an}$, but also the trend in the deviation between the two temperatures, cast for convenience into $T_{max,sim}/T_{max,an}-1$ in Fig.~\ref{fig:final4plots}-b, is directly related to the total amount of heat $H_{tot}$ (orange) produced in the device.
%E: no \frac inline should be used, AIP requirement
...
To visualize this changing shape, we plot the relative change of heat density, $H_{\kappa}(z)/H_{\kappa_{ref}(z)}-1$ (red curve), that occurs upon going from $\kappa$ = 1 (reference) to $10^{-4}$~\thermalconductivity, as a function of the position $z$ in the electrically active region in Fig.~\ref{fig:final4plots}-d .
...
%E:I am not sure we have to explain all the things below. Basically, it is important to show that profile changes is also important. Therefore i propose next way:[Due to the arc-shaped temperature distribution across the OLED, the lowering of $\kappa$ causes the temperature to rise mainly in the center of the device and to a much lesser extent near the outer surfaces. While the temperate elevation increases the recombination and the electric conductivity at the device center, the electrical conductivity near the surfaces considerably lesser raised.Since Joule heating is more pronounced in the regions with lower electrical conductivity, more heat is produced near the surface than in the center.]
That means, heating profile changes form when $\kappa$ changes. Heating increases more towards the device surfaces, therefore becoming more and more homogeneous. Therefore, strange coincidence of simulation results and analytic solution is explained by the changes of heating profile, and in general, the analytic equations still should not be used for such low thermal conductivities. Nevertheless, this raises question, why we don't see the same effect when we change heat transport coefficient?
...
To explain this, we also show the relative change of heat density, $H_{h}(z)/H_{h_{ref}}(z)-1$, when changing the heat transfer coefficient from $h$ = $10^{5}$ (reference) to $10^{3.5}$~\hcoefficient~(Fig.~\ref{fig:final4plots}-d, black curve).
...
Hence, the lowering/increase of $h$ rather preserves the shape of $H(z)$.
